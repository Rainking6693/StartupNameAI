...
import TestPremiumButton from './TestPremiumButton';

const ResultsPageImproved = () => {
  ...
  const getScoreBadge = (score) => {
    ...
    if (score >= 8) return { icon: Star, label: 'Excellent', color: 'bg-blue-500/20 text-blue-300' };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-pink-800">
      ...
      {/* Test Premium Button (for development) */}
      {process.env.NODE_ENV === 'development' && <TestPremiumButton />}
    </div>
  );
};
...
const FreemiumModal = ({ show, onClose, onUpgrade, totalNames, freeNamesShown, isUpgrading }) => {
  if (!show) return null;
          ...
          <div className="text-center">
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full bg-gradient-to-r from-yellow-500 to-orange-500 text-white px-6 py-3 rounded-xl font-bold hover:from-yellow-600 hover:to-orange-600 transition-all duration-300 disabled:opacity-50"
              >
                {isUpgrading ? (
...
export default ResultsPageImproved;
